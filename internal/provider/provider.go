// Package provider определяет контракт провайдера нейросети и реестр,
// который подбирает провайдера под указанную модель.
//
// Чтобы подключить новую нейросеть, достаточно реализовать Provider
// и зарегистрировать реализацию в реестре (см. internal/app/app.go).
package provider
...
// Provider — нейросеть (или её API), к которой сервис умеет отправлять запросы.
type Provider interface {
	// Name — уникальное имя провайдера; попадает в ответ API.
	Name() string
